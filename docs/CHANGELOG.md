...
## [2026-04-05] - 混合检索 A/B 测试完成

### 🎯 功能增强

**B3 任务完成：混合检索 A/B 测试**

创建混合检索 A/B 测试框架，对比纯向量检索 vs 混合检索（向量+BM25）效果。

**测试结果**：
| 配置 | 关键词覆盖率 | 响应时间 |
|------|-------------|---------|
| vector_dominant (0.7/0.3) | 87% | 0.401s |
| balanced (0.5/0.5) | 93% | 0.664s |
| **bm25_dominant (0.3/0.7)** | **97%** | 0.336s |

**推荐配置**：`bm25_dominant`（关键词优先），关键词覆盖率达 97%。

**代码变更**：
| 文件 | 变更 |
|------|------|
| `tests/integration/test_hybrid_ab.py` | 新增 - 混合检索 A/B 测试文件 |
| `docker-compose.dev.yml` | 修改 - 添加 tests 目录挂载 |
