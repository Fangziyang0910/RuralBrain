...
| - 测试多模态消息处理 | | | | |
| **A2: 网络搜索工具开发** | A | 3天 | - | 待开始 |
| - 实现 `src/agents/tools/web_search_tool.py` | | | | |
| - 创建 `src/agents/skills/configs/web_search.yaml` | | | | |
| - 注册到工具加载器 | | | | |
| **B1: RAG 与搜索融合** | B | 2天 | A2 | 待开始 |
| - 在 `planning.yaml` 中集成搜索工具 | | | | |
| - 实现混合检索策略 | | | | |
| - 优化检索结果排序 | | | | |
| **C1: 定价工具集成搜索** | C | 2天 | A2 | 待开始 |
| - 修改 `pricing_tool.py` 调用搜索获取实时价格 | | | | |
| - 实现搜索结果过滤和验证 | | | | |
| - 对比本地知识库和网络价格 | | | | |
| **C2: 检测多模态增强** | C | 2天 | A1 | 待开始 |
| - 检测结果用 Qwen-VL 生成自然语言解释 | | | | |
| - 提供处理建议和预防措施 | | | | |
| - 增强前端展示 | | | | |
| **D1: 巡检图片分析增强** | D | 2天 | A1 | 待开始 |
| - `farm_inspection_tool` 支持多图片输入 | | | | |
| - Qwen-VL 综合分析多张巡检照片 | | | | |
| **D2: 疾病预测多模态** | D | 2天 | A1 | 待开始 |
