...
export const ChatMessageBubble: React.FC<ChatMessageBubbleProps> = ({
  ...
  const displayContent = isUser ? message.content : normalizeMarkdownForRendering(message.content);
  const hasDiseaseToolReport = !isUser && message.toolCalls?.some(
    (toolCall) => toolCall.name === "disease_prediction_tool" && toolCall.resultText
  );
  const hasVisibleContent = isUser || message.isStreaming || (!hasDiseaseToolReport && displayContent.trim().length > 0);
              ...
              <div key={idx} className="animate-slide-up" style={{ animationDelay: `${idx * 0.1}s` }}>
                ...
                {toolCall.name === "disease_prediction_tool" && toolCall.resultData && (
                  <div className="mt-2 space-y-2">
                    ...
                    {toolCall.resultText && (
                      <div className="message-bubble message-ai-enhanced">
                        <MarkdownContent content={toolCall.resultText} />
                      </div>
                    )}
                  </div>
                )}
