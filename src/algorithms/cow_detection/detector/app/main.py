...
try:
    ...
    from src.algorithms.cow_detection.detector.app.api.routes import router as api_router
...
app = FastAPI(
    ...
    基于先进的YOLOv8深度学习模型，为现代农业提供快速、准确的牛只识别服务。
    
    ## 🎯 核心功能
    - **智能识别**: 支持多种常见牛只品种的自动识别
    - **高精度检测**: 基于YOLOv8模型，检测准确率高达95%+
    - **实时处理**: 1-3秒内完成图像分析和结果返回
    - **多目标检测**: 同时识别图像中的多个不同牛只
    - **可视化结果**: 返回带有标注框的处理后图像
    ...
    **版权信息**: © 2024 智能农业科技团队
    ...
    contact={
        ...
        "email": "support@cow-detection.com"
    },
    license_info={
        "name": "MIT License",
...
def custom_openapi():
    ...
    openapi_schema["tags"] = [
        {
            "name": "牛只检测",
            "description": "🐄 核心牛只识别功能，支持多种常见牛只品种的智能检测"
        },
        {
            "name": "系统信息", 
...
def root():
    ...
    这是一个基于YOLOv8深度学习模型的智能牛只检测服务，为现代农业提供准确、快速的牛只识别能力。
    
    ## 🚀 快速开始
    1. 访问 [API文档](/docs) 查看详细使用说明
    2. 使用 `/detect` 接口上传图像进行检测
    3. 查看 `/supported-cows` 了解支持的牛只类型
    ...
    return {
        "service_name": "🐄 智能牛只检测API",
        "version": settings.VERSION,
        "description": "基于YOLOv8的智能牛只检测服务",
        "status": "🟢 运行中",
        "features": {
            "supported_cows": "多种常见牛只品种",
        ...
        "contact": {
            "support_email": "support@cow-detection.com",
